...
for csv_file in csv_files:
    ...
    # 数据清洗：删除每个district中租金超过均价三倍标准差的数据
    cleaned_df = pd.DataFrame()  # 初始化一个空的DataFrame，用于存储清洗后的数据
    ...
    for district in df['district'].unique():
        ...
        # 计算统计信息
        avg_price = district_df['price_per_m2'].mean()
        std_dev_price = district_df['price_per_m2'].std()
        if district == '金沙洲':
            print(avg_price)
            print(std_dev_price)
        # 对每个district进行三倍标准差的清洗
        cleaned_district_df = district_df[abs(district_df['price_per_m2'] - avg_price) <= 2 * std_dev_price]

        # 将清洗后的数据添加到总的清洗数据中
        cleaned_df = cleaned_df.append(cleaned_district_df, ignore_index=True)
