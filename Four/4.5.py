...
for csv_file in csv_files:
    ...
    for room_type in [1, 2, 3]:
        ...
        # 计算中位数
        median_price = df_room['total_price'].median()

        # 存储数据
        city_data['城市'].append(city_name)
        city_data['户型'].append(f'{room_type}居室')
        city_data['中位数'].append(median_price)
...
# Bar plot for median prices of one-bedroom, two-bedroom, and three-bedroom apartments for each city
sns.barplot(x='城市', y='中位数', hue='户型', data=city_df)

# 设置图表标题和标签
plt.title('各城市不同户型的租金中位数')
plt.xlabel('城市')
plt.ylabel('租金中位数(元)')
