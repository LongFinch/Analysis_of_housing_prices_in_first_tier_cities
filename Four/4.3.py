...
for csv_file in csv_files:
    ...
    for room_type in [1, 2, 3]:
        ...
        # 计算最低价
        min_price = df_room['total_price'].min()

        # 存储数据
        city_data['城市'].append(city_name)
        city_data['户型'].append(f'{room_type}居室')
        city_data['最低价'].append(min_price)
...
# Bar plot for minimum prices of one-bedroom, two-bedroom, and three-bedroom apartments for each city
sns.barplot(x='城市', y='最低价', hue='户型', data=city_df)

# 设置图表标题和标签
plt.title('各城市不同户型的最低租金')
plt.xlabel('城市')
plt.ylabel('最低租金(元)')
